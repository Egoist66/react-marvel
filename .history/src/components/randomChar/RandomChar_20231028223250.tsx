...
class RandomChar extends Component<any, RandomCharState> {
    ...
    private updateChar = async () => {
        const id = 1011005
        const {data: {results}} = await service.getCharacter(id)
        ...
        this.setState({
            name: results[0].name,
            description: results[0].description || 'No data',
            thumbnail: results[0].thumbnail.path,
            wiki: results[0].urls.find(u => u.type === 'wiki')?.url,
            homepage: results[0].urls.find(u => u.type === 'wiki')?.url,
        })
        ...
        return (
            <StyledRandomChar className="randomchar">
                <div className="randomchar__block">
                    {!thumbnail ? <p>No image</p> : <img src={thumbnail} alt="Random character" className="randomchar__img"/>}
