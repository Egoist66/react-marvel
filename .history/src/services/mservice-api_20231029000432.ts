import {DataRootCharacters, DataWrapper} from "../app-types/types.ts";
...
class MService extends MDataService {
    ...
    async getCharacter(id: () => RandomID) {
        ...
        try {
            const char: DataWrapper = await this.getResource(`${this._apiBase}characters/${id()}?${this._apiKey}`, 'json')
            return this.transformCharacterData(char)
